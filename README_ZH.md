...
位移步长是数值精度参数，不是对所有材料通用的常数。中心有限差分的主导截断误差
正比于 `h²`，所以最初减小 `h` 会提高精度；但力噪声和浮点相消误差会沿相反方向
增长。计算 `n` 阶力常数需要对力做 `n-1` 次位移微分，力噪声可能近似按
`h^{-(n-1)}` 放大，因此四阶及更高阶对过小步长尤其敏感。

可以把以下范围作为起点：光滑解析经典势使用 `0.001–0.003 Å`，高精度机器学习势
使用 `0.003–0.01 Å`，充分收敛的电子结构力从约 `0.01 Å` 开始。这些不是保证最优的
固定值。生产计算至少应在候选值附近测试三个步长，比较关键张量分量、整体范数或下游
物理量，选择截断误差已经下降而力噪声尚未开始增长的平台区。近邻截断和超胞尺寸需要
另外独立做收敛检查。

解析 FC4 CI 选用 ASE 的单参数组 Morse calculator，是因为它允许构造独立的精确四阶
导数。它能在几何复杂、含多个原子的结构上运行，但不会区分不同化学元素对，因此不能
作为一般多组分材料的物理势。多组分覆盖由 K4As4Pt2 机器学习势参考测试另行完成。
