...
def _bands(supercell: Path, force_constants: Path):
...
def main() -> None:
    path, asr = _bands(ROOT / "input/reference.vasp", ROOT / "results/asr/FORCE_CONSTANTS_2ND")
    constrained_path = ROOT / "results/born-huang-huang/FORCE_CONSTANTS_2ND"
    constrained = _bands(ROOT / "input/reference.vasp", constrained_path)[1] if constrained_path.is_file() else None
    ticks, labels = [], []
