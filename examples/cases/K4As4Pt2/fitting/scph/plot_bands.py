...
REFERENCE = HERE.parent / "reference.vasp"
PRIMITIVE = HERE.parent / "primitive.vasp"
...
def _phonon(force_constants: Path) -> Phonopy:
    ...
    primitive_matrix = np.asarray(read(PRIMITIVE).cell) @ np.linalg.inv(np.asarray(cell.cell))
    phonon = Phonopy(cell, np.eye(3, dtype=int), primitive_matrix=primitive_matrix)
...
def main() -> None:
    sources = [("Harmonic", HERE.parent / "anharmonic" / "three-body" / "FORCE_CONSTANTS_2ND")]
    sources.extend(
        (f"Loop-SCPH {temperature} K", HERE / f"T{temperature}" / "FORCE_CONSTANTS_2ND")
    ...
    for axis, (title, source) in zip(axes.flat, sources, strict=True):
        ...
        if title.startswith("Loop-SCPH"):
            ...
            status = json.loads((HERE / f"T{temperature}" / "history.json").read_text())
    ...
    figure.savefig(HERE / "phonopy-seekpath-harmonic-vs-scph.png", dpi=180, bbox_inches="tight")
