...
不表示 MLFCS 恢复了命令行接口。
新增脚本和材料案例应遵循[测试与案例设计原则](../docs/zh/development/tests-and-examples.md)。

## 直接使用 calculator

- [`basic_fc2.py`](basic_fc2.py) 使用 ASE EMT 弛豫并计算小体系，然后写出 FC2；
- [`nep89_orders.py`](nep89_orders.py) 通过 calorine 加载用户提供的 NEP89 模型，计算
...
[`vasp_external_fc3.py`](vasp_external_fc3.py) 给出完整的三阶段案例：
...
在使用外部力之前请阅读[完整工作流](../docs/zh/workflows/external-calculators.md)。
