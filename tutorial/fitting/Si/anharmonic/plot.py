"""Plot Si FC2–FC4 Taylor-fit harmonic phonon bands along a SeeK-path path."""
...
def main() -> None:
    ...
    if cell is None:
        raise ValueError("cannot read Si reference supercell")
    ...
    path = seekpath.get_path(
        (np.asarray(primitive.cell), primitive.scaled_positions, primitive.numbers), recipe="hpkot"
    )
    paths = [
    ...
    for distance, values in zip(band.distances, band.frequencies, strict=True):
        axis.plot(distance, np.asarray(values), color="#a34e25", linewidth=0.9)
    ticks, labels = [], []
    for (start, end), distance in zip(path["path"], band.distances, strict=True):
        ticks.extend((float(distance[0]), float(distance[-1])))
        labels.extend((start, end))
    axis.set_xticks(ticks, labels)
    axis.set_ylabel("Frequency (THz)")
    axis.set_title("Si FC2–FC4 Taylor-fit harmonic phonon bands")
