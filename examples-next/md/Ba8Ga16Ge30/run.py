...
def _force_calculator(fcp: ForceConstantPotential, reference):
    """Build a calculator tied to the exact repeated reference frame."""
    return ForceConstantCalculator(fcp.get_force_constants(reference.repeat(SUPERCELL)))
...
def run_temperature(temperature: int, fcp: ForceConstantPotential, overwrite: bool) -> None:
    ...
    atoms = reference.repeat(SUPERCELL)
