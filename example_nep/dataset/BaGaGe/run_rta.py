#!/usr/bin/env python3
"""Test BaGaGe FC2/FC3 loading and calculate BTE-RTA conductivity."""
...
def main() -> None:
    ...
    parser.add_argument("--supercell", type=Path, default=CASE / "original/supercell.vasp")
    ...
    parser.add_argument("--mesh", nargs=3, type=int, default=(3, 3, 3))
    parser.add_argument("--temperatures", nargs="+", type=float, default=(300.0,))
    parser.add_argument("--boundary-mfp", type=float, default=1.0, metavar="MICROMETER")
    parser.add_argument(
        "--isotope", action=argparse.BooleanOptionalAction, default=True
    )
    ...
    print(
        f"Loaded {len(phono3py.primitive)} primitive atoms, "
        f"mesh={tuple(args.mesh)}, temperatures={tuple(args.temperatures)}"
    )
