...
def calculate_distances(
    supercell: dict,
) -> tuple[NDArray[np.float64], NDArray[np.intc], NDArray[np.intc]]:
    """
    计算超胞中所有原子对之间的距离
    ...
    cartesian_positions = np.dot(supercell["lattvec"], supercell["positions"])
    ...
    equivalent_count = is_equivalent.sum(axis=0, dtype=np.intc)
...
def displace_three_atoms(
    ...
    Args:
        ...
        atom_k: 第三个原子索引
        coord_k: 第三个原子的位移坐标方向 (0, 1, 2)
        displacement_k: 第三个原子的位移量 (nm)
    ...
    result["positions"][:, atom_i] += scipy.linalg.solve(
        result["lattvec"],
        displacement
    )
    ...
    result["positions"][:, atom_j] += scipy.linalg.solve(
        result["lattvec"],
        displacement
    )
    ...
    # 位移第三个原子
    displacement[:] = 0.0
    displacement[coord_k] = displacement_k
    result["positions"][:, atom_k] += scipy.linalg.solve(
        result["lattvec"],
        displacement
    )
...
def generate_supercell(
    ...
    # 扩展晶格矢量
    supercell["lattvec"] = np.array(primitive["lattvec"])
    supercell["lattvec"][:, 0] *= na
    supercell["lattvec"][:, 1] *= nb
    supercell["lattvec"][:, 2] *= nc
    
    supercell["elements"] = copy.copy(primitive["elements"])
    supercell["numbers"] = na * nb * nc * primitive["numbers"]
    
    num_primitive_atoms = primitive["positions"].shape[1]
    num_supercell_atoms = num_primitive_atoms * na * nb * nc
    supercell["positions"] = np.empty((3, num_supercell_atoms))
    ...
    for _ in range(na * nb * nc):
        supercell["types"].extend(primitive["types"])
...
# 向后兼容的别名
calc_dists = calculate_distances
calc_frange = calculate_cutoff_range
move_two_atoms = displace_two_atoms
move_three_atoms = displace_three_atoms
gen_SPOSCAR = generate_supercell
_parse_cutoff = parse_cutoff
_validate_cutoff = validate_supercell_size
