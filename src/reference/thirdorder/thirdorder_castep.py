...
def gen_CASTEP_supercell(CASTEP_cell, na, nb, nc):
    ...
    for pos, (k, j, i, iat) in enumerate(
        itertools.product(
            ...
            xrange(nb),
            xrange(na),
            xrange(CASTEP_cell["positions"].shape[1]),
        )
    ):
...
def normalize_CASTEP_supercell(CASTEP_supercell):
    ...
    indices = np.array(xrange(nruter["positions"].shape[1])).reshape(
        (CASTEP_supercell["nc"], CASTEP_supercell["nb"], CASTEP_supercell["na"], -1)
    )
...
if __name__ == "__main__":
    if len(sys.argv) != 7 or sys.argv[1] not in ("sow", "reap"):
        sys.exit(
            "Usage: {0} sow|reap na nb nc cutoff[nm/-integer] seedname".format(
                sys.argv[0]
            )
        )
    action = sys.argv[1]
    ...
    wedge = thirdorder_core.Wedge(
        CASTEP_cell, CASTEP_supercell, symops, dmin, nequi, shifts, frange
    )
    ...
    if action == "sow":
        ...
        for i, e in enumerate(list4):
            for n in xrange(4):
                ...
                displaced_CASTEP_supercell = normalize_CASTEP_supercell(
                    move_two_atoms(
                        CASTEP_supercell, e[1], e[3], isign * H, e[0], e[2], jsign * H
                    )
                )
        ...
        for i in sorted(glob.glob(cell_names)):
            ...
            if not glob.glob("%s.param" % seedname):
                sys.exit(
                    "%s.param " % seedname + "was not found."
