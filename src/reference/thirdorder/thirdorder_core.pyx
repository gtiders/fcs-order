...
cdef inline int _ind2id(int[:] icell,int ispecies,int[:] ngrid,int nspecies):
    ...
    """
    # 说明（Python/公式对应）：
    # 给定三维晶格索引 icell = (ix, iy, iz)，以及原胞内的原子/物种索引 ispecies，
    # 在尺寸为 ngrid = (na, nb, nc) 的超晶胞中，将四元组 (ix, iy, iz, ispecies)
    # 展平成一维索引 id 的标准写法是：
    #   id = (((iz * nb) + iy) * na + ix) * nspecies + ispecies
    # 这与下面实现的等式完全等价：
    #   (ix + (iy + iz * ngrid[1]) * ngrid[0]) * nspecies + ispecies
    # 逐项对应关系：
    #   iz * nb <=> iz * ngrid[1]
    #   ((iz * nb) + iy) * na + ix <=> ix + (iy + iz * ngrid[1]) * ngrid[0]
    # 最后乘以 nspecies 并加上 ispecies，用于在每个晶格点展开 nspecies 个原子/物种。
