...
def run(
    ...
    if action == "sow":
        ...
        StructureData.from_dict(norm_sposcar).to_file("4TH.SPOSCAR", out_format="vasp")
...
def calculate_force_constants(
    ...
    h: float = None,
    symprec: float = None,
    output_filename: str = "FORCE_CONSTANTS_4TH",
    verbose: bool = True,
):
    """
    计算四阶力常数的库函数接口。
    ...
    Args:
        ...
        h: 位移幅度 (nm)，默认 1e-3
        symprec: 对称性容差，默认 1e-5
        output_filename: 输出力常数文件名，默认 "FORCE_CONSTANTS_4TH"
        verbose: 是否打印进度信息
    ...
    for i, e in enumerate(list6):
        for n in range(8):
            ...
            ksign = (-1) ** (n % 2)
            number = nirred * n + i
            phipart[:, i, :] -= isign * jsign * ksign * sorted_forces[number].T
    phipart /= 8000.0 * h * h * h
