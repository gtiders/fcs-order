...
class Structure:
    ...
    def to_file(self, filename: str, out_format: str = "vasp") -> None:
        """
        Write structure to file using ASE (VASP format only).

        Args:
            filename: Output file path.
            out_format: Output format (must be 'vasp').
        """
        if out_format.lower() != "vasp":
            raise ValueError("Only 'vasp' format is supported for file writing.")

        atoms = self.to_atoms()

        # Write file directly without modifying extension
