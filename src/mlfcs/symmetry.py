...
class SymmetryOperations:
    ...
    def _get_symmetry_dataset(self):
        ...
        # NOTE: spglib expects lattice as column vectors, so we transpose
        # Our internal representation uses row vectors (lattvec[i] is i-th lattice vector)
        lattice = self._lattvec.T
        cell = (lattice, self._positions, self._types)
        ...
        # Extract symmetry information (use attribute access, not dict access)
        ...
        for i in range(self.nsyms):
            ...
            self._crotations[i] = np.dot(
                self._lattvec, np.dot(self._rotations[i], lat_inv)
            )
    ...
    def apply_all(self, r_in):
        ...
        for isym in range(self.nsyms):
            ...
            r_out[:, isym] = (
                np.dot(self._crotations[isym], r_in) + self._ctranslations[isym]
            )
