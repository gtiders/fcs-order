...
def hiphive(
    ...
    sposcar: str = typer.Option(
        "SPOSCAR",
        "--sposcar",
        help="Path to a structure file parsable by ASE (e.g., VASP POSCAR, CIF, XYZ). Default: 'SPOSCAR'",
        exists=True,
    ),
):
    """
    ...
    typer.secho(
        "Ensure that the SPOSCAR file you pass is generated by phonopy using the same expansion matrix as used for FCP training.",
        fg=typer.colors.RED,
    )
    try:
