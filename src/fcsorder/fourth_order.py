...
"""CLI for calculating fourth-order force constants using ML potentials."""
...
from fcsorder.io.writer import write_ifcs4
...
def create_displaced_structure_three_atoms(
    structure_dict: dict,
    atom_i: int,
    coord_i: int,
    ...
    """Create a copy of structure with three atoms displaced.

    Displaces three atoms along specified Cartesian directions. The displacements
    ...
    Args:
        ...
        atom_k: Index of third atom to displace.
        coord_k: Cartesian direction (0=x, 1=y, 2=z) for third atom.
        displacement_k: Displacement magnitude in nm for third atom.
    ...
    # Displace third atom
    cartesian_disp[:] = 0.0
    cartesian_disp[coord_k] = displacement_k
    fractional_disp = scipy.linalg.solve(lattice, cartesian_disp)
    displaced["positions"][:, atom_k] += fractional_disp

    return displaced


def prepare_fc4_calculation(
    ...
    """Prepare data structures for fourth-order force constant calculation.
...
def calculate_fc4(
    ...
    """Calculate fourth-order force constants using fourthorder algorithm.
    ...
    Returns:
        None: Force constants are saved to FORCE_CONSTANTS_4TH file.
    """
    ...
    (
        ...
        force_range,
        n_neighbors,
    ) = prepare_fc4_calculation(structure, supercell_matrix, cutoff, symprec)

    n_atoms = len(primitive_dict["types"])
    n_total = n_atoms * na * nb * nc

    # Build wedge and list of quartets
    wedge = fourthorder_core.QuartetWedge(
        primitive_dict,
        supercell_dict,
        symmetry_ops,
        min_distances,
    ...
    quartet_list = wedge.build_list4()
    n_irreducible = len(quartet_list)
    n_runs = 8 * n_irreducible
    ...
    supercell_atoms.write("4TH.SPOSCAR.xyz", format="extxyz")

    # Setup for displaced structures
    width = len(str(8 * (len(quartet_list) + 1)))
    name_pattern = f"4TH.POSCAR.{{:0{width}d}}.xyz"

    typer.echo("Computing an irreducible set of anharmonic force constants")
    phi_partial = np.zeros((3, n_irreducible, n_total))

    with Progress() as progress:
        task = progress.add_task("Processing quartets", total=len(quartet_list))
        for i, quartet in enumerate(quartet_list):
            for n in range(8):
                sign_i = (-1) ** (n // 4)
                sign_j = (-1) ** (n % 4 // 2)
                sign_k = (-1) ** (n % 2)
                ...
                displaced_dict = create_displaced_structure_three_atoms(
                    supercell_dict,
                    quartet[2],
                    quartet[5],
                    sign_i * h,
                ...
                phi_partial[:, i, :] -= sign_i * sign_j * sign_k * forces.T
    ...
    phi_partial /= 8000.0 * h * h * h

    typer.echo("Reconstructing the full array")
    phi_full = fourthorder_core.reconstruct_ifcs(
        phi_partial, wedge, quartet_list, primitive_dict, supercell_dict
    )
...
def fc4(
    ...
    """
    Calculate fourth-order force constants using any registered calculator.

    Example:
        fc4 2 2 2 --calculator nep --potential model.nep --cutoff -3
        fc4 2 2 2 --calculator dp --potential model.pb --cutoff 0.5
        fc4 2 2 2 --calculator tace --potential model.pt --cutoff -4 --device cuda
    """
    ...
    supercell_atoms = supercell_structure.to_atoms()
    supercell_atoms.write("fc4_SPOSCAR", format="vasp", direct=True)
    typer.echo(f"Supercell matrix: {na} x {nb} x {nc}")
    typer.echo("Supercell written to fc4_SPOSCAR")
    ...
    # Run fourth-order force constants calculation
    typer.echo("Starting fourth-order force constants calculation...")
    try:
        calculate_fc4(
            structure=structure,
            calc=calc,
        ...
        typer.secho(
            "✓ Fourth-order force constants calculation completed successfully",
