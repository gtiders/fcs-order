...
class MTP(Calculator):
    ...
    def __init__(self, mtp_path="pot.mtp", mtp_exe="mlp", 
                 tmp_folder=None, unique_elements=None, **kwargs):
...
def read_cfg(file, symbols):
    ...
    for block in block_pattern.findall(lines):
        ...
        virial_stress = (
            -np.array(list(map(formatify, stress_str.split()))).reshape(6,) / volume
        )
