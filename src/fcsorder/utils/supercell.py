...
try:  # pragma: no cover - passthrough import
    from ..core.secondorder_core import build_supercell_from_matrix as build_supercell_from_matrix  # type: ignore
...
def parse_supercell_matrix(supercell_matrix: List[int]) -> np.ndarray:
    ...
    if len(supercell_matrix) not in (3, 9):
        raise ValueError(
            "Supercell matrix must have 3 (diagonal) or 9 (3x3) integers"
        )
