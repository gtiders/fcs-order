...
下面的例子使用 EMT 生成 24 个随机位移结构。实际使用时，通常可以直接从已有数据文件中读取训练结构，例如：
...
更多高阶力常数拟合方法见[力常数拟合](仅力数据的力常数拟合.md)。

有限差分的完整接口见[有限差分 API](../参考/有限差分API.md)，拟合器参数和诊断信息见[拟合 API](../参考/拟合API.md)。
